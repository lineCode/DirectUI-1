...
void DirectUI_API DUI__Trace(LPCTSTR pstrFormat, ...)
{
	...
	LPTSTR lpBuffer = (TCHAR *)malloc( (nLen + 1) * sizeof(TCHAR) );
	ZeroMemory(lpBuffer, (nLen + 1) * sizeof(TCHAR));
	_vsntprintf(lpBuffer, nLen + 1, pstrFormat, Args);
	va_end(Args);
    
	strMsg.Assign(lpBuffer);
	free(lpBuffer);

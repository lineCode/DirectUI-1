...
DWORD CControlUI::GetNativeBkColor() const
{
	return m_dwNativeBkColor;
}

void CControlUI::SetNativeBkColor(DWORD dwColor)
{
	m_dwNativeBkColor = dwColor;
	Invalidate();
}
...
void CControlUI::SetVirtualWnd(LPCTSTR pstrValue)
{
	...
	if(m_pManager) m_pManager->UsedVirtualWnd(true);
}
...
void CControlUI::PaintBkColor(HDC hDC)
{
	if (IsFocused())
	{
		if (m_dwNativeBkColor != 0)
		{
			CRenderEngine::DrawColor(hDC, m_rcPaint, GetAdjustColor(m_dwNativeBkColor));
		}
		else
		{

...
CStdPtrArray CPaintManagerUI::m_aGlobalTransAccel;
...
CPaintManagerUI::~CPaintManagerUI()
{
	...
	// Reset other parts...
	if( m_hwndTooltip != NULL ){ ::DestroyWindow(m_hwndTooltip); m_hwndTooltip = NULL;}
	...
	if( m_hDcPaint != NULL ){ ::ReleaseDC(m_hWndPaint, m_hDcPaint); m_hDcPaint = NULL;}
    RemovePreMessage();
}
...
bool CPaintManagerUI::SetTimer(LPCTSTR strControl, UINT nTimerID, UINT uElapse)
{
	CControlUI *pControl=FindControl(strControl);
	return SetTimer(pControl,nTimerID, uElapse);
}
 
bool CPaintManagerUI::KillTimer(LPCTSTR strControl, UINT nTimerID)
{
	CControlUI *pControl=FindControl(strControl);
	return KillTimer(pControl,nTimerID);
}
 
void CPaintManagerUI::KillTimer(LPCTSTR strControl)
{
	CControlUI *pControl=FindControl(strControl);
	KillTimer(pControl);
}
...
bool CPaintManagerUI::SetTimer(CControlUI* pControl, UINT nTimerID, UINT uElapse)
{
    ...
    for (int i = 0; i< m_aTimers.GetSize(); i++) 
	{
		...
		{
            //if (pTimer->bKilled||pTimer->uElapse!=uElapse)
			{
...
bool CPaintManagerUI::SetNextTabControl(bool bForward)
{
    ...
    if(pControl) {
		SetFocus(pControl);
		if (_tcsstr(pControl->GetClass(), _T("EditUI")) != NULL ) {
			CEditUI *pEd=dynamic_cast<CEditUI *>(pControl);
			if (pEd) {
				pEd->SetSelAll();
			}
		}
...
bool CPaintManagerUI::AddPreMessage()
{
	if(-1 == m_aPreMessages.Find(this))
		return m_aPreMessages.Add(this);
	
	return true;
}

bool CPaintManagerUI::RemovePreMessage()
{
	return m_aPreMessages.Remove(m_aPreMessages.Find(this));
}
...
const TImageInfo* CPaintManagerUI::GetImageEx(LPCTSTR lpszBitmap, LPCTSTR lpszType, DWORD dwMask)
{
    ...
    if (!pImageInfo) 
	{
		...
		if(!pImageInfo && m_pParentResourcePM) //parent support
		{
			pImageInfo =  const_cast<TImageInfo*>(m_pParentResourcePM->GetImageEx(lpszBitmap,lpszType,dwMask));
		}
    }
...
void CPaintManagerUI::ReMoveAllImagesToAother(CPaintManagerUI* pOther)
{
	 TImageInfo* pImageInfo = NULL;
    ...
    for (int i = 0; i< m_mImageHash.GetSize(); i++) 
	{
		...
		{
            pImageInfo = static_cast<TImageInfo*>(m_mImageHash.Find(key, false));
			if(pOther->GetImage(key)) //have image alrealy
			{
				//continue; //but copy to update
			}
			if(pImageInfo) //add to new manager
			{
				pOther->AddImage(key,pImageInfo->hBitmap,pImageInfo->nX,pImageInfo->nY,pImageInfo);
				m_mImageHash.Remove(key);
				i--;
			}
        }
    }
...
bool CPaintManagerUI::TranslateMessage(const LPMSG pMsg)
{
	...
	//pretranslate global acceleraror
	for (int n = 0; n < m_aGlobalTransAccel.GetSize(); n++) 
	{
		ITranslateAccelerator* pT = static_cast<ITranslateAccelerator*>(m_aGlobalTransAccel[n]);        
	...
	else
	{
		...
		{
			CPaintManagerUI* pT = static_cast<CPaintManagerUI*>(m_aPreMessages[i]);
			if (pT&&pMsg->hwnd == pT->GetPaintWindow())
			{
				if (pT->TranslateAccelerator(pMsg)) return true;
				if (pT->PreMessageHandler(pMsg->message, pMsg->wParam, pMsg->lParam, lRes)) return true;
...
bool CPaintManagerUI::AddGlobalTranslateAccelerator(ITranslateAccelerator *pTranslateAccelerator)
{
	ASSERT(m_aGlobalTransAccel.Find(pTranslateAccelerator) < 0);
	return m_aGlobalTransAccel.Add(pTranslateAccelerator);
}

bool CPaintManagerUI::RemoveGlobalTranslateAccelerator(ITranslateAccelerator *pTranslateAccelerator)
{
	for (int i = 0; i < m_aGlobalTransAccel.GetSize(); i++)
	{
		if (static_cast<ITranslateAccelerator *>(m_aGlobalTransAccel[i]) == pTranslateAccelerator)
		{
			return m_aGlobalTransAccel.Remove(i);
		}
	}

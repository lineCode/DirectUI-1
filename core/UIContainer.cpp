...
namespace DirectUI
{
	...
	void LoopControl(CControlUI* pControl, std::vector<CActiveXUI*>& refAtx)
	{
		if (pControl == NULL) return;
		auto IsActiveX=[&](CControlUI*& pCtl)->void
		{
			CActiveXUI* pActiveX = static_cast<CActiveXUI*>(pCtl->GetInterface(DUI_CTR_ACTIVEX));
			if(pActiveX == NULL) return;
			refAtx.push_back(pActiveX);
		};
		CContainerUI* pContainer = static_cast<CContainerUI*>(pControl->GetInterface(DUI_CTR_CONTAINER));
		if(pContainer == NULL) //是否是容器
		{
			IsActiveX(pControl);
			return;
		}
		...
		{
			LoopControl(pContainer->GetItemAt(it),refAtx);
		}
	}

	//为浮动(Docker)窗口设计
	//遍历所有控件如果是ACTIVEX的话将ACTIVEX的父窗口重置为其他的父窗口
	//Add 将ACTIVEX父窗口设为CContainerUI的绘画窗口,并显示
	//RemoveNotDestroy 将ACTIVEX父窗口设为NULL即桌面,并隐藏
	void CContainerUI::LoopAllControl(CControlUI* pControl, bool add)
	{
		std::vector<CActiveXUI*> vecAtx;
		//loop ctls
		if (pControl == NULL) return ;
		LoopControl(pControl,vecAtx);
		
		//更新所有ACTIVEX窗口
		for(int it=0;it<vecAtx.size();it++)
		{
			ASSERT(vecAtx[it]);
			vecAtx[it]->ChangeHostParentWindow( (add && CContainerUI::m_pManager != NULL)  ? CContainerUI::m_pManager->GetPaintWindow() : NULL);
		}
	}

	//并不销毁的移除控件,移除后必须添加到另一控件上或手动销毁
	bool CContainerUI::RemoveNotDestroy(CControlUI* pControl)
	{
		if (pControl == NULL) return false;
		for (int it = 0; it < m_items.GetSize(); it++)
		{
			...
			{
				NeedUpdate();
				bool ret = m_items.Remove(it);
				LoopAllControl(pControl,false);
				return ret;
			}
		}
		return false;
	...
	void CContainerUI::MoveItem(int nSrcIndex, int nDesIndex)
	{

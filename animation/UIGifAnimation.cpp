...
namespace DirectUI 
{
	...
	void CGifAnimation::ClearGifData()
	{
		m_bOnlyFirstFrame=FALSE;
		if (m_pGifData) {
			delete []m_pGifData;
	...
	const TImageInfo* CGifAnimation::LoadGIFFirstFrame(const LPBYTE pData, DWORD dwSize, DWORD mask/* = 0*/)
	{
		if (!pData || dwSize==0)
			return NULL;
		...
		TGIFHeader* pGIFHeader = (TGIFHeader*)pData;

		if ((memcmp(&pGIFHeader->m_cSignature, "GIF", 3) != 0)
			&& ((memcmp(&pGIFHeader->m_cVersion, "87a", 3) != 0)
			|| (memcmp(&pGIFHeader->m_cVersion, "89a", 3) != 0)))
		{
			// it's neither GIF87a nor GIF89a
			// do nothing
			// clear GIF variables
		...
		m_pGifData=new unsigned char[dwSize];
		memcpy(m_pGifData, pData, dwSize);
		m_nGifSize=dwSize;

		m_pRawData=m_pGifData;
		m_nDataSize=dwSize;
		m_pGIFLSDescriptor = (TGIFLSDescriptor*)(m_pRawData + sizeof(TGIFHeader));
		if (m_pGIFLSDescriptor->GetPackedValue(LSD_PACKED_GLOBALCT) == 1)
		{
	...
	bool CGifAnimation::Play()
	{
		...
		if (m_bOnlyFirstFrame) {
			if (LoadGIF(m_pGifData, m_nGifSize, m_nGifMask)==NULL)
				return false;
		}
	...
	{
		CGifAnimation* pAniImage = new CGifAnimation(this);
		if (!pAniImage->LoadGIFFirstFrame(pData, dwSize, mask))
		{
			delete pAniImage;
			return NULL;
		}
	...
	CLabelEx::CLabelEx()
		...
		,m_dwIndex(0)
		,m_pNotify(NULL)
		,m_bMousePlayEnabled(true)
		,m_bAutoPlayEnabled(false)
	{
	...
	{
		if (_tcscmp(pstrName,_T("mouseplay"))==0) {
			if (_tcscmp(pstrValue, _T("true")) == 0) {
				SetMousePlayEnabled(true);
				SetAutoPlayEnabled(false);
			} else {
				SetMousePlayEnabled(false);
			}
		} else if (_tcscmp(pstrName,_T("autoplay"))==0) {
			if (_tcscmp(pstrValue, _T("true")) == 0) {
				SetAutoPlayEnabled(true);
				SetMousePlayEnabled(false);
				m_bPlaying=Play(GetBkImage());
			} else {
				SetAutoPlayEnabled(false);
			}
		} else
	...
	void CLabelEx::DoEvent(TEventUI& event)
	{
		if (event.Type == UIEVENT_MOUSEENTER)
		{
			if (m_pNotify)
			{
				...
				notify.sType = DUI_MSGTYPE_MOUSEENTER;
				notify.ptMouse = m_pManager->GetMousePos();
				m_pNotify->Notify(notify);
			}

			CUIString strName = GetBkImage();
			if (!strName.IsEmpty()&&IsMousePlayEnabled())
			{
				m_bPlaying=Play(strName);
			}
			return;
		}
		...
		if (event.Type == UIEVENT_MOUSELEAVE)
		{
			if (m_pNotify)
			{
				...
				notify.pSender = this;
				notify.wParam = m_dwIndex;
				notify.sType = DUI_MSGTYPE_MOUSELEAVE;
				notify.ptMouse = m_pManager->GetMousePos();
				m_pNotify->Notify(notify);
			}
		...
		if (event.Type == UIEVENT_DBLCLICK)
		{
			if (m_pNotify)
			{
				...
				notify.pSender = this;
				notify.wParam = m_dwIndex;
				notify.sType = DUI_MSGTYPE_ITEMDBCLICK;
				notify.ptMouse = m_pManager->GetMousePos();
				m_pNotify->Notify(notify);
			}
		}
	...
	bool CLabelEx::IsPlaying()
	{
		return m_bPlaying;
	}

	void CLabelEx::CheckAutoPlay()
	{
	...
	void CLabelEx::SetVisible(bool bVisible)
	{
		__super::SetVisible(bVisible);
		CheckAutoPlay();
	}

	void CLabelEx::SetPos(RECT rc)
	{
		__super::SetPos(rc);
		CheckAutoPlay();
	}

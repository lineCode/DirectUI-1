...
namespace DirectUI
{
	CScrollBarUI::CScrollBarUI() : m_bHorizontal(false), m_nRange(100), m_nScrollPos(0), m_nLineSize(8), m_nThumbFixedSize(DEFAULT_SCROLLBAR_SIZE),
		m_pOwner(NULL), m_nLastScrollPos(0), m_nLastScrollOffset(0), m_nScrollRepeatDelay(0), m_uButton1State(0), \
		m_uButton2State(0), m_uThumbState(0), m_bShowButton1(true), m_bShowButton2(true),m_bShowEmptyRect(true)
	{
	...
	int CScrollBarUI::GetThumbFixedSize() const
	{
		return m_nThumbFixedSize;
	}

	void CScrollBarUI::SetThumbFixedSize(int nSize)
	{
		m_nThumbFixedSize = nSize;
	}
		...
		if( m_bHorizontal ) {
			...
			if( cx > m_cxyFixed.cy ) {
				...
				if( m_nRange > 0 ) {
					...
					if( cxThumb < m_cxyFixed.cy ) cxThumb = m_nThumbFixedSize; //设置拇指按钮的最小长度. org: m_cxyFixed.cy
			...
			if( cy > m_cxyFixed.cx ) {
				...
				if( m_nRange > 0 ) {
					...
					if( cyThumb < m_cxyFixed.cx ) cyThumb = m_nThumbFixedSize;// m_cxyFixed.cx; 
		...
		if( event.Type == UIEVENT_MOUSEMOVE )
		{
			if( (m_uThumbState & UISTATE_CAPTURED) != 0 ) {
				if( !m_bHorizontal ) {
					...
					if( m_bShowButton1 ) vRange -= m_cxyFixed.cx;
					if( m_bShowButton2 ) vRange -= m_cxyFixed.cx;
					...
					//当按钮没有显示时,不需要减去(高/宽)度
					if( m_bShowButton1 ) hRange -= m_cxyFixed.cy;
					if( m_bShowButton2 ) hRange -= m_cxyFixed.cy;
	...
	void CScrollBarUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName, _T("thumbsize")) == 0 ) SetThumbFixedSize(_ttoi(pstrValue));
		...
		else if( _tcscmp(pstrName, _T("showemptybuttonrect")) == 0 ) m_bShowEmptyRect = _tcscmp(pstrValue, _T("true")) == 0;

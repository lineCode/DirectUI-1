...
// 				m_pOwner->GetManager()->SendNotify(m_pOwner, DUI_MSGTYPE_KILLFOCUS);
		...
		else if( uMsg == WM_KEYDOWN && TCHAR(wParam) == VK_ESCAPE)
		{
			...
			if(!dwTextColor) dwTextColor = m_pOwner->GetTextColor();
	...
	CEditUI::CEditUI() : m_pWindow(NULL), m_uMaxChar(255), m_bReadOnly(false), 
		...
		m_dwEditbkColor(0xFFFFFFFF), m_iWindowStyls(0)/*, m_bNotifyFocusOut(false)*/
		,m_bDBFocuse(false),m_bReturnEnd(false),m_bEscEnd(false),m_dwEditTextColor(0x0)
	{
	...
	void CEditUI::DoEvent(TEventUI& event)
	{
		...
		if (event.Type == UIEVENT_KILLFOCUS && IsEnabled()) 
		{
			...
			if (pControl)
			{
				CUIString strClassName = pControl->GetClass();
				if (_tcscmp(strClassName, _T("EditSliderUI")) == 0)
				{
					CEditSliderUI* pEditSlide = static_cast<CEditSliderUI*>(pControl);
					m_pManager->SendNotify(pEditSlide, DUI_MSGTYPE_KILLFOCUS);
				}
			}
		}
		if (event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK || event.Type == UIEVENT_RBUTTONDOWN) 
		{
			if (IsEnabled()) 
			{
				...
				if(m_bDBFocuse && event.Type != UIEVENT_DBLCLICK) return;
...
// 	bool CEditUI::IsNotifyFocusOut() const
// 	{
// 		return m_bNotifyFocusOut;
// 	}

// 	void CEditUI::SetNotifyFocusOut(bool bNotify)
// 	{
// 		m_bNotifyFocusOut= bNotify;
// 	}

	bool CEditUI::IsDoubleClickFocus() const
	{
		return m_bDBFocuse;
	}

	void CEditUI::SetDoubleClickFocus(bool bFlag)
	{
		m_bDBFocuse= bFlag;
	}

	bool CEditUI::IsEnterEndEdit() const
	{
		return m_bReturnEnd;
	}

	void CEditUI::SetEnterEndEdit(bool bFlag)
	{
		m_bReturnEnd= bFlag;
	}

	bool CEditUI::IsEscEndEdit() const
	{
		return m_bEscEnd;
	}

	void CEditUI::SetEscEndEdit(bool bFlag)
	{
		m_bEscEnd= bFlag;
	}
	...
	void CEditUI::SetEditTextColor(DWORD dwTextColor)
	{
		m_dwEditTextColor = dwTextColor;
	}

	DWORD CEditUI::GetEditTextColor() const
	{
...
// 		else if( _tcscmp(pstrName, _T("notifyfocusout")) == 0 ) {
// 			SetNotifyFocusOut( _tcscmp(pstrValue, _T("true")) == 0 );
// 		}
		else if( _tcscmp(pstrName, _T("edittextcolor")) == 0 ) {
			if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
			LPTSTR pstr = NULL;
			DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
			SetEditTextColor(clrColor);
		}
		else if( _tcscmp(pstrName, _T("dbfocuse")) == 0 ) SetDoubleClickFocus(_tcscmp(pstrValue, _T("true")) == 0);
		else if( _tcscmp(pstrName, _T("returnend")) == 0 ) SetEnterEndEdit(_tcscmp(pstrValue, _T("true")) == 0);
		else if( _tcscmp(pstrName, _T("escend")) == 0 ) SetEscEndEdit(_tcscmp(pstrValue, _T("true")) == 0);

...
bool  CActiveXUI::ChangeHostParentWindow(HWND hWnd)
{
	if(m_hwndHost)
	{
#ifdef __UIWEBBROWSER_H__
		CWebBrowserUI* pWebBrowser = static_cast<CWebBrowserUI*>(this->GetInterface(DUI_CTR_WEBBROWSER));
		if(pWebBrowser && hWnd == NULL)
			GetManager()->RemoveTranslateAccelerator(pWebBrowser);
#endif
		//if(hWnd==NULL) this->SetVisible(false); //桌面附加
		::SetParent(m_hwndHost,hWnd);
		//if(hWnd!=NULL) this->SetVisible(true); //其他窗口附加
#ifdef __UIWEBBROWSER_H__
		if(pWebBrowser && hWnd != NULL)
		{
			GetManager()->AddTranslateAccelerator(pWebBrowser);
			pWebBrowser->NeedParentUpdate();
		}
#endif
	}
	return true;

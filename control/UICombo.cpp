...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if( uMsg == WM_CREATE ) 
	{
        ...
        for( int i = 0; i < m_pOwner->GetCount(); i++ ) {
			IListItemUI* pListItem = static_cast<IListItemUI*>(m_pOwner->GetItemAt(i)->GetInterface(_T("ListItem")));
			if(pListItem != NULL)
    ...
    else if( uMsg == WM_MOUSEWHEEL ) 
	{
		...
		if(!m_pOwner->GetMouseWheelSelect())
		{
			if(m_pOwner->GetItemAt(0))
			{
				CRect rc = m_pOwner->GetItemAt(0)->GetPos();
				Scroll(0, zDelta < 0 ? rc.GetHeight() : (rc.GetHeight() * -1));
			}
		}
		else
		{
...
{
    if (m_pWindow != NULL&&!IsMouseWheeling()) m_pWindow->Close();
...
bool CComboUI::SelectRange(int nIndex, bool bTakeFocus /* = false */)
{
	return true;
}
...
void CComboUI::MoveSelectedItemToHotItem()
{
	return;
}

void CComboUI::SetHotItemIndex(int nIndex)
{
	return;
}

void CComboUI::SetMouseState(bool bDown)
{
	return;
}
...
void CComboUI::SetMouseWheeling(BOOL b)
{
	m_bMouseWheeling=b;
}

BOOL CComboUI::IsMouseWheeling()
{
	return m_bMouseWheeling;
}

void CComboUI::SetMouseWheelSelect(bool isMouseWheelSelect)
{
	m_bMouseWheelSelect = isMouseWheelSelect;
}

bool CComboUI::GetMouseWheelSelect()
{

...
CListUI::CListUI() : m_pCallback(NULL), m_bScrollSelect(false), m_bCombinationList(false)
					,m_iCurSel(-1), m_iExpandedItem(-1),m_bKeySelectHorizontal(false),m_bMultiSelect(false)
					,m_bListItemDrag(false),m_iHotItemIndex(-1),m_bMouseState(false),m_nArrowKeysVIndex(0),m_nArrowKeysHIndex(0)
...
void CListUI::SelectAll()
{
	...
	{
		CControlUI* pControl = GetItemAt(n);
		if (pControl)
		{
			if (pControl->IsVisible() && pControl->IsEnabled())
			{
				...
				if (pListItem)
				{
					...
					pListItem->SetLastSelected(true);
					m_iCurSel = n;
				}
			}
		}
	...
	if (m_pManager)
	{
		m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMSELECT, 0, GetCount());
	}
}


void CListUI::MoveUp(int nIndex)
{
	if (nIndex<=0 || GetCount()<=nIndex) return;

	CControlUI* pControl = GetItemAt(nIndex);
	CControlUI* pControlUp = GetItemAt(nIndex-1);
	if (pControl && pControl->IsVisible() && pControl->IsEnabled()&&pControlUp && pControlUp->IsVisible() && pControlUp->IsEnabled())
	{
		IListItemUI* pListItem = static_cast<IListItemUI*>(pControl->GetInterface(_T("ListItem")));
		if (pListItem)
		{
			pListItem->SetIndex(nIndex-1);
		}
		IListItemUI* pListItemUp = static_cast<IListItemUI*>(pControlUp->GetInterface(_T("ListItem")));
		if (pListItemUp)
		{
			pListItemUp->SetIndex(nIndex);
		}
	}
	m_pList->MoveItem(nIndex,nIndex-1);
	SelectItem(nIndex-1);
}


void CListUI::MoveDown(int nIndex)
{
	if (nIndex<0 || GetCount()-1<=nIndex) return;

	CControlUI* pControl = GetItemAt(nIndex);
	CControlUI* pControlDown = GetItemAt(nIndex+1);
	if (pControl && pControl->IsVisible() && pControl->IsEnabled()&&pControlDown && pControlDown->IsVisible() && pControlDown->IsEnabled())
	{
		IListItemUI* pListItem = static_cast<IListItemUI*>(pControl->GetInterface(_T("ListItem")));
		if (pListItem)
		{
		...
		IListItemUI* pListItemUp = static_cast<IListItemUI*>(pControlDown->GetInterface(_T("ListItem")));
		if (pListItemUp)
		{
			pListItemUp->SetIndex(nIndex);
		}
...
void CListUI::DoEvent(TEventUI& event)
{
	...
	{
		m_nArrowKeysVIndex = 0;
		m_nArrowKeysHIndex = 0;
		return;
	}
	...
	if (event.Type == UIEVENT_TIMER && event.wParam == DEFAULT_TIMERID)
	{
		...
		else if (pt.y >= rt.bottom-20)
		{
			m_pList->LineDown();
		}
		else if (pt.x <= rt.left+20)
		{
			m_pList->LineLeft();
		}
		else if (pt.x >= rt.right-20)
		{
			m_pList->LineRight();
		}
		return;
    ...
    case UIEVENT_KEYDOWN:
		...
		{
        case VK_UP:
			if (GetSelectHorizontal()) {
				if (m_pList) {
					if (GetKeyState(VK_SHIFT) < 0)
					{
						--m_nArrowKeysHIndex;
						if (m_iCurSel+m_nArrowKeysHIndex<0)
						{
							m_nArrowKeysHIndex = -m_iCurSel;
						}
						SelectRange(m_iCurSel+m_nArrowKeysHIndex, true);
					}
					else
					{
						m_nArrowKeysHIndex = 0;
						...
						if (m_iCurSel - nCol>=0)
							SelectItem(FindSelectable(m_iCurSel - nCol, false), true);
					}
				}
			} else {
				if (GetKeyState(VK_SHIFT) < 0)
				{
					--m_nArrowKeysVIndex;
					if (m_iCurSel+m_nArrowKeysVIndex<0)
					{
						m_nArrowKeysVIndex = -m_iCurSel;
					}
					SelectRange(m_iCurSel+m_nArrowKeysVIndex, true);
				}
				else
				{
					m_nArrowKeysVIndex = 0;
			...
			if (m_pManager && IsEnabled())
			{
				CListContainerElementUI* pElement = static_cast<CListContainerElementUI*>(GetItemAt(m_iCurSel));
				if (pElement)
				{
					m_pManager->SendNotify(pElement, DUI_MSGTYPE_ITEMCLICK);
				}
			}
            return;
        case VK_DOWN:
			if (GetSelectHorizontal()){
				if (m_pList) {
					...
					{
						++m_nArrowKeysHIndex;
						if (m_iCurSel+m_nArrowKeysHIndex >= GetCount())
						{
							m_nArrowKeysHIndex = GetCount()-m_iCurSel-1;
						}
						SelectRange(m_iCurSel + m_nArrowKeysHIndex, true);
					}
					else
					{
						m_nArrowKeysHIndex = 0;
						int nCol=m_pList->GetColumns();
						if (m_iCurSel + nCol<GetCount())
							SelectItem(FindSelectable(m_iCurSel + nCol, true), true);
					}
				}
			}else{
				if (GetKeyState(VK_SHIFT) < 0)
	            {
					++m_nArrowKeysVIndex;
					if (m_iCurSel+m_nArrowKeysVIndex >= GetCount())
					{
						m_nArrowKeysVIndex = GetCount()-m_iCurSel-1;
					}
					SelectRange(m_iCurSel + m_nArrowKeysVIndex, true);
	            }
				else
...
bool CListUI::SelectItem(int nIndex, bool bTakeFocus)
{
	...
	bool bMultiSelect = GetMultiSelect();

	if (!bCtrlKey || !bMultiSelect)
	{
		int nCount = GetCount();
			...
			if (n != nIndex)
			{
				...
				if (pControl)
				{
					...
					if (pListItem)
					{
						...
						pListItem->SetLastSelected(false);
					}
				}
			}
		}
	...
	{
		bool bLastSelected = pListItem->GetLastSelected();
		if (!pListItem->Select(!bLastSelected, false))
		{
			m_iCurSel = -1;
			return false;
		}
		pListItem->SetLastSelected(!bLastSelected);
	}
...
bool CListUI::SelectRange(int nIndex, bool bTakeFocus /* = false */)
{
	int i=0;
	...
	if (m_pManager)
	{
		if (bMultiSelect)
		{
			m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMSELECT, nIndex, m_iCurSel);
		}
		else
		{
			m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMSELECT, nIndex, nIndex);
		}
	}
...
void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    else if( _tcscmp(pstrName, _T("itemalign")) == 0 ) 
	{
		...
		if( _tcsstr(pstrValue, _T("vleft")) != NULL ) {
            m_ListInfo.uTextStyle &= ~(DT_CENTER | DT_RIGHT);
            m_ListInfo.uTextStyle |= DT_LEFT|DT_VCENTER;
        }
        if( _tcsstr(pstrValue, _T("vcenter")) != NULL ) {
            m_ListInfo.uTextStyle &= ~(DT_LEFT | DT_RIGHT);
            m_ListInfo.uTextStyle |= DT_CENTER|DT_VCENTER;
        }
        if( _tcsstr(pstrValue, _T("vright")) != NULL ) {
            m_ListInfo.uTextStyle &= ~(DT_LEFT | DT_CENTER);
            m_ListInfo.uTextStyle |= DT_RIGHT|DT_VCENTER;
        }
    }
	...
	else if (_tcscmp(pstrName, _T("combinationlist")) == 0 )
	{
 	...
 	else if (_tcscmp(pstrName, _T("selecthorizontal")) == 0) {
		SetSelectHorizontal(_tcscmp(pstrValue, _T("true")) == 0);
	} 
	else if (_tcscmp(pstrName, _T("multiselect")) == 0)
	{
		SetMultiSelect(_tcscmp(pstrValue,_T("true"))==0);
	}
	else if (_tcscmp(pstrName, _T("dragitem")) == 0)
	{
		SetListItemDrag(_tcscmp(pstrValue,_T("true"))==0);
	}
	else if (_tcscmp(pstrName, _T("ensurevisible")) == 0) //选中的时候是否移动位置
	{
		if(_tcscmp(pstrValue, _T("true")) == 0)
			m_bEnsureVisible = true;
...
bool CListUI::GetSelectHorizontal()const
{
	return m_bKeySelectHorizontal;
}

void CListUI::SetSelectHorizontal(bool bSel)
{
	m_bKeySelectHorizontal=bSel;
}
...
bool CListUI::GetMultiSelect() const
{
	return m_bMultiSelect;
}


void CListUI::SetListItemDrag(bool bDrag)
{
	m_bListItemDrag = bDrag;
}


bool CListUI::GetListItemDrag() const
{
	return m_bListItemDrag;
}


void CListUI::MoveSelectedItemToHotItem()
{
	if (!GetListItemDrag())
	{
		return;
	...
	int nHotItemIndex = GetHotItemIndex();
	if (nHotItemIndex<0)
	{
		return;
	}

	if (m_iCurSel == nHotItemIndex)
	{
		return;
	}
		...
		if (pControl)
		{
			if (pControl->IsVisible() && pControl->IsEnabled())
			{
				...
				if (pListItem)
				{
					...
					{
						mapSelected.insert(pair<int,bool>(n, false));
					}
					else if (GetCurSel() == n)
					{
						pListItem->Select(true, false);
	...
	int nMin = -1;
	int nMax = -1;
	for (map<int,bool>::iterator it=mapSelected.begin(); it!=mapSelected.end(); ++it)
	{
		if (nMin == -1)
		{
			nMin = it->first;
		...
		if (it->first==nHotItemIndex)
		{
			return;
		}
	}

	if (nHotItemIndex>=nMin && nHotItemIndex<=nMax)
	{
		return;
	}

	int nCurItemIndex = nHotItemIndex;
	...
	for (map<int,bool>::iterator it=mapSelected.begin(); it!=mapSelected.end(); ++it)
	{
		...
		if (nItemSelected>nCurItemIndex)
		{
			for (int n=nCurItemIndex; n<=nItemSelected; ++n)
			{
				CControlUI* pControl = GetItemAt(n);
				if (pControl && pControl->IsVisible() && pControl->IsEnabled())
				{
					...
					if (pListItem)
					{
						if (n==nItemSelected)
						{
							pListItem->SetIndex(nCurItemIndex);
						}
						else
						{
							pListItem->SetIndex(n+1);
						}
					}
				}
			}
			m_pList->MoveItem(nItemSelected, nCurItemIndex);
			++nCurItemIndex;
			it->second = true;
		}
	}
	...
	//Move dwon
	for (map<int,bool>::reverse_iterator it=mapSelected.rbegin(); it!=mapSelected.rend(); ++it)
	{
		int nItemSelected = -1;
		if (!(it->second))
		{
		...
		if (nItemSelected<nCurItemIndex)
		{
			for (int n=nItemSelected; n<=nCurItemIndex; ++n)
			{
				CControlUI* pControl = GetItemAt(n);
				if (pControl && pControl->IsVisible() && pControl->IsEnabled())
				{
...
void CListUI::SetHotItemIndex(int nIndex)
{
	m_iHotItemIndex = nIndex;
}


int CListUI::GetHotItemIndex() const
{
	return m_iHotItemIndex;
}


void CListUI::SetMouseState(bool bDown)
{
	if (!IsEnabled() && m_pManager!=NULL)
	{
		return;
	}
	if (bDown == m_bMouseState)
	{
		return;
	}
	...
	for( int it1 = 0; it1 < m_items.GetSize(); it1++ ) 
	{
		...
		if ((iCount % m_nColumns) == 0)
		{
			...
			for (int it2 = it1; it2 < m_items.GetSize(); it2++) 
			{
				...
				if (!pLineControl||!pLineControl->IsVisible()) continue;
...
UINT CListHeaderItemUI::GetButtonState() const
{
...
bool CListElementUI::Select(bool bSelect, bool bCallback)
{
    ...
    if( bSelect == m_bSelected) {
		if (!m_pOwner||!m_pOwner->NeedCurSelectNotify())
			return true;
	}
    m_bSelected = bSelect;
...
bool CListLabelElementUI::GetLastSelected() const
{
	return true;
}


bool CListLabelElementUI::SetLastSelected(bool bLastSelected)
{
	return true;
}
...
bool CListTextElementUI::GetLastSelected() const
{
	return true;
}

bool CListTextElementUI::SetLastSelected(bool bLastSelected)
{
	return true;
}
...
bool CListContainerElementUI::Select(bool bSelect, bool bCallback)
{
	...
	bool bResult = true;
	if (m_pOwner && bCallback)
	{
		if (bShiftKey)
		{
			bResult = m_pOwner->SelectRange(m_iIndex);
		}
		else
		{
			bResult = m_pOwner->SelectItem(m_iIndex);
		}
	}
...
bool CListContainerElementUI::SetLastSelected(bool bLastSel)
{
	return (m_bLastSelected=bLastSel);
}


bool CListContainerElementUI::GetLastSelected() const
{
	return m_bLastSelected;
}
...
void CListContainerElementUI::DoEvent(TEventUI& event)
{
    ...
    if (event.Type == UIEVENT_BUTTONDOWN)
    {
        if (IsEnabled())
		{
			...
			if (m_pOwner)
			{
				m_pOwner->SetMouseState(true);
			}
            SetItemDragState(Select());
            Invalidate();
			if (m_pManager)
				m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMCLICK);
    ...
    if (event.Type == UIEVENT_MOUSEENTER)
    {
        if (IsEnabled()) 
		{
			if (m_pOwner)
			{
				m_pOwner->SetHotItemIndex(m_iIndex);
			}
            m_uButtonState |= UISTATE_HOT;
            Invalidate();
        }
...
void CListContainerElementUI::SetItemDragState(bool bState)
{
	m_bDragState = bState;
}


bool CListContainerElementUI::GetItemDragState() const
{
	return m_bDragState;
}


void CListContainerElementUI::DragListItem()
{
	if (!m_pOwner)
	{
		return;
	...
	if (GetItemDragState())
	{
		SetItemDragState(false);
		m_pOwner->MoveSelectedItemToHotItem();
	}
}

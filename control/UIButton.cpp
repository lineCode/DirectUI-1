	...
	void CButtonUI::DoEvent(TEventUI& event)
	{
		...
		if( event.Type == UIEVENT_BUTTONUP )
		{
			if( (m_uButtonState & UISTATE_CAPTURED) != 0 ) {
				...
				m_uButtonState &= ~(UISTATE_PUSHED | UISTATE_CAPTURED);
				m_pManager->SendNotify(this, DUI_MSGTYPE_BUTTONUP, event.wParam, event.lParam);
				Invalidate();
			}
			return;
		}
...
Label_ForeImage:
	...
	void CButtonUI::ChangeNormalImageToOther(BTN_IMGS nStatus)
	{
		CUIString strNormal;
	...
	void CButtonUI::FlashBtn(int nTimerID, int nTime)
	{
		if (m_pManager) {
			m_pManager->SetTimer(this,nTimerID,nTime);
			m_nFlashTimerID=nTimerID;
		}
	}
	void CButtonUI::StopFlash()
	{
		if (m_pManager&&m_nFlashTimerID!=0) {
			m_pManager->KillTimer(this,m_nFlashTimerID);
			m_nFlashTimerID=0;
			ChangeNormalImageToOther(BTN_IMG_NORMAL);
		}
	}
	void CButtonUI::OnFlashTimer()
	{
		int nNow=m_nNowNormalID+1;
		if (nNow>BTN_IMG_PUSHED)
			nNow=BTN_IMG_NORMAL;
		ChangeNormalImageToOther((BTN_IMGS)nNow);
	}

	void CButtonUI::SetHandCursor(BOOL bHand)
	{
		m_bHandCursor=bHand;
	}
	
	BOOL CButtonUI::IsHandCursor()const
	{

...
//				m_pManager->SendNotify(this, _T("itemdbclick"));
	...
	void CTreeNodeUI::SetParentNode( CTreeNodeUI* _pParentTreeNode )
	{
		...
		if (_pParentTreeNode!=NULL) {
			m_iTreeLevel=_pParentTreeNode->GetTreeLevel()+1;
		} else
			m_iTreeLevel=0;
	}
	...
	void CTreeNodeUI::SetToolTip(LPCTSTR strTip)
	{
		this->pHoriz->SetToolTip(strTip);
	}
	/*****************************************************************************/
	/*****************************************************************************/
	/*****************************************************************************/
		...
		if(_TreeNode)
		{
			if(_TreeNode->GetCountChild() > 0)
			{
				...
				for(int nIndex = 0;nIndex < nCount;nIndex++)
				{
					...
					pItem->SetVisibleTag(_Expanded);
				//	pItem->GetFolderButton()->SetCheck(!_Expanded);
